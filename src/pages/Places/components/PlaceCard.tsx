    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="grid grid-cols-3 gap-2 text-center mt-auto">
          ...
          <Stat icon="reviews" value={`${place.reviewCount}`} label="đánh giá" />
        </div>
      </div>
    </Link>
  )
...
function Stat({ icon, value, label }: { icon: string; value: string; label: string }) {
  return (
    <div className="bg-surface-container-low rounded-xl py-2 px-1.5">
      <Icon name={icon} size={14} className="text-on-surface-variant" />
      <p className="text-[11px] font-bold text-on-surface mt-0.5 leading-tight truncate">{value}</p>

    ...
    >
        ...
        {guide.highlights && guide.highlights.length > 0 && (
          <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
            {guide.highlights.slice(0, 2).map((h) => (
              <span
                key={h}
                className="px-2.5 py-1 rounded-full bg-white/90 backdrop-blur text-[10px] font-bold uppercase tracking-wider text-primary shadow"
              >
                {h}
              </span>
            ))}
      ...
      <div className="px-5 pt-9 pb-5 flex flex-col flex-grow">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-center">
          ...
          <Stat icon="schedule" value={guide.responseTime ?? '-'} label="phản hồi" />
        </div>

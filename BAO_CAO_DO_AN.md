...
- Gợi ý chuyến đi cá nhân hoá bằng **thuật toán chấm điểm có trọng số** kết hợp 3 nguồn: độ hợp sở thích (`user_preferences` + `User.preferences`), tương tác cá nhân (xem/click/lưu/yêu cầu tham gia) và độ phổ biến toàn hệ thống.
...
- Chưa có module gợi ý theo machine learning thực sự (collaborative filtering); thuật toán gợi ý hiện theo mô hình **chấm điểm có trọng số** kết hợp sở thích, tương tác cá nhân và độ phổ biến.
...
CSDL gồm **24 bảng chính** chia thành 6 nhóm:

**Nhóm 1 – User & Auth (4 bảng):**
- `users` — tài khoản (id, email, passwordHash, role, displayName, avatar, cover, bio, preferences JSONB)
- `user_preferences` — sở thích chi tiết phục vụ gợi ý cá nhân hoá (userId UNIQUE, categories[], interests[], provinces[], budgetTier)
...
**Nhóm 3 – Trips (5 bảng):**
- `trips` — chuyến đi (id, ownerId, title, slug, destination, startDate, endDate, budget, gallery, itinerary JSONB, status, viewCount, clickCount, requestCount)
- `trip_members` — thành viên (tripId, userId, role, joinedAt)
- `trip_join_requests` — yêu cầu tham gia (tripId, userId, status, message)
- `trip_interactions` — tương tác cá nhân phục vụ gợi ý (userId, tripId, views, clicks, favorited, requested)
...
### 4.5.5. Thuật toán gợi ý chuyến đi có trọng số

Ngoài gợi ý qua trợ lý AI, hệ thống còn có **bộ gợi ý chuyến đi (recommendation)** chạy ở endpoint `GET /api/trips/recommended`. Khác với phiên bản đầu chỉ chấm điểm theo sở thích, bộ gợi ý hiện kết hợp **ba nguồn tín hiệu** rồi tổng hợp bằng công thức trọng số:

```
final = 0.3 × match + 0.3 × interaction + 0.4 × hot
```

Mỗi thành phần được **chuẩn hoá min–max về [0, 1]** trên tập chuyến ứng viên trước khi nhân trọng số, đảm bảo ba nguồn tín hiệu có cùng thang đo:

- **match (độ hợp sở thích – 30%):** so khớp chuyến với hồ sơ sở thích người dùng. Nguồn dữ liệu gồm cột `users.preferences` (JSONB: phong cách, ngân sách…) và bảng chuyên biệt `user_preferences` (danh mục yêu thích, từ khoá quan tâm, tỉnh hay đi). Điểm cộng khi: danh mục chuyến trùng sở thích (+3), mỗi tag trùng (+2), điểm đến trùng tỉnh ưa thích (+3).
- **interaction (tương tác cá nhân – 30%):** mức độ chính người dùng đã quan tâm chuyến đó, lấy từ bảng `trip_interactions` (số lần xem, click, đã lưu, đã gửi yêu cầu tham gia). Giúp "kéo lên" những chuyến người dùng từng tương tác.
- **hot (độ phổ biến toàn hệ thống – 40%):** mức độ "nóng" của chuyến với toàn bộ cộng đồng, tính từ các bộ đếm trên bảng `trips` (`view_count`, `click_count`, `request_count`) và số thành viên. Trọng số cao nhất để ưu tiên chuyến đang được nhiều người quan tâm.

**Thu thập tín hiệu tương tác:** hai endpoint `POST /api/trips/:id/view` và `POST /api/trips/:id/click` được gọi khi người dùng mở chi tiết hoặc click thẻ chuyến. Chúng vừa tăng bộ đếm tổng (phục vụ `hot`), vừa ghi nhận theo từng cặp (người dùng, chuyến) vào `trip_interactions` (phục vụ `interaction`). Việc gửi yêu cầu tham gia cũng cộng `request_count` và đánh dấu cờ tương tác.

Kết quả trả về kèm `recommendReasons` (lý do gợi ý: "Hợp gu Biển", "Đúng nơi bạn thích", "Đang hot"…) và `scoreBreakdown` (điểm từng thành phần) để giao diện giải thích "vì sao gợi ý này", tăng tính minh bạch cho người dùng.

```
GET /api/trips/recommended
       ↓
Tải sở thích (users.preferences + user_preferences)
Tải tương tác cá nhân (trip_interactions của user)
Lấy chuyến đã publish, còn hạn (tối đa 120 ứng viên)
       ↓
Với mỗi chuyến: tính matchRaw, interactionRaw, hotRaw
       ↓
Chuẩn hoá min–max từng thành phần về [0,1]
       ↓
final = 0.3·match + 0.3·interaction + 0.4·hot
       ↓
Sắp xếp giảm dần → trả top N kèm lý do + breakdown
```
...
| UC06 | Gợi ý có trọng số | set sở thích biển + Đà Nẵng | Trip hợp gu lên top, kèm lý do + điểm | Đúng | ✅ |
| UC06 | Tương tác đẩy gợi ý | xem/click 1 trip nhiều lần | Điểm interaction tăng, trip lên hạng | Đúng | ✅ |
...
- **Recommendation engine:** nâng cấp từ chấm điểm có trọng số (hiện tại) lên ML model (collaborative filtering với matrix factorization) để học hành vi ngầm.
...
| Trip (gợi ý) | `/api/trips/recommended`, `/api/trips/:id/view`, `/api/trips/:id/click` | GET/POST |
| User (sở thích) | `/api/users/me/preferences` | GET/PUT |
...
24 bảng đã liệt kê ở Chương 4.2. Mỗi bảng đều có:
